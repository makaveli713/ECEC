...
namespace Art713.Project713.Cryptography
{
    ...
    public class Encryption
    {
        ...
        /// <summary>
        /// When a numeric value of the bytes (which user want to encrypt)
        /// is greater than modulus of the given elliptic curve
        /// array divides on parts and BytesPartListWithSign contains 
        /// every part of this array with boolean value, 
        /// which show if the BigInteger representation of the part 
        /// is less than zero
        /// </summary>
        public Dictionary<byte[], bool> BytesPartListWithSign = new Dictionary<byte[], bool>();
        /// <summary>
        /// Elliptic curve.
        ...
        /// <summary>
        /// This method allow to divide bytes array
        /// in the case if BigInteger representation
        /// of this array is greater than the modulus
        /// of the given elliptic curve.
        /// </summary>
        /// <param name="text">Bytes array to encrypt.</param>
        public void GetParts(byte[] text)
        {
            var textLength = text.Length;
            var boundFlag = 0;
            var bound = textLength / 2;
            ...
            var part1 = new byte[bound];
            var part2 = new byte[bound-boundFlag];

            for (var i = 0; i < part1.Length; i++)            
                part1[i] = text[i];
            for (var i = 0; i < part2.Length; i++)            
                part2[i] = text[i+bound];

            var part1BigInteger = new BigInteger(part1);
            var part2BigInteger = new BigInteger(part2);
            ...
            else
            {
                if (part1BigInteger < EllipticCurve.P && part2BigInteger < EllipticCurve.P)
                {
                    BytesPartListWithSign.Add(part1, f1);
                    BytesPartListWithSign.Add(part2, f2);
                }
                else
                {
                    if (part1BigInteger > EllipticCurve.P && part2BigInteger < EllipticCurve.P)
        ...
        /// <summary>
        /// method: Allow to encrypt bytes array.
        /// </summary>
        /// <param name="bytesToEncrypt">Bytes array to encrypt.</param>
        /// <returns>Encrypted text and R point as a string</returns>
        public string Encrypt(byte[] bytesToEncrypt)
        {
            #region Elliptic curve generation
            // только для тестирования!!!
            const int a = -3;
            var b = BigInteger.Parse("5ac635d8aa3a93e7b3ebbd55769886bc651d06b0cc53b0f63bce3c3e27d2604b", System.Globalization.NumberStyles.HexNumber);
            var p = BigInteger.Parse("115792089210356248762697446949407573530086143415290314195533631308867097853951");
            ...
            var textToEncryptBytesArray = bytesToEncrypt;
            var textToEncryptBigInteger = new BigInteger(textToEncryptBytesArray);
            if (textToEncryptBigInteger < 0)
            {
                textToEncryptBigInteger *= -1;
            ...
            GetParts(bytesToEncrypt);

            var s = BytesPartListWithSign
                .Select(sign => sign.Value)
                .Aggregate(string.Empty, (current, sign) => sign ? current + "1" : current + "0") + "-";

                s += BytesPartListWithSign
                    .Select(part => (part.Value) ? -1 * (new BigInteger(part.Key)) : new BigInteger(part.Key))
                    .Select(part => part * pPoint.Abscissa)
                    .Select(part => Auxiliary.Math.Mod(part, EllipticCurve.P))
                    .Aggregate(string.Empty, (current, part) => current + part.ToString() + "+");
        ...
        public byte[] DecryptAndReturnBytesArray(string encryptedText)
        {
            var signes = encryptedText.Split('-');
            var encryptedTextStringArray = signes[1].Split(' ');
            ...
            for (var i = 0; i < parts.Length; i++)
            {
                ...
                if (signesCharArr[i] == '1')
                    part *= -1;
                list.Add(part.ToByteArray());
            }

            var length = list.Sum(bytesArr => bytesArr.Length);
            var returnedArr = new byte[length];

            var k = 0;
            foreach (var element in list.SelectMany(bytesArr => bytesArr))
            {
                returnedArr[k] = element;
                k++;
            }
